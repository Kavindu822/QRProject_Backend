...
namespace QRFileTrackingapi.Controllers
{
    ...
    public class RcodeFilesController : ControllerBase
    {
        ...
        // Search files by partial Rcode
        [HttpGet("search/{query}")]
        public async Task<IActionResult> SearchFilesByRcode(string query)
        {
            ...
            var matchingFiles = await dbContext.RcodeFiles
                .Where(f => f.Rcode.Contains(query))
                .Select(f => new
                {
                    f.Rcode,
                    f.EpfNo,
        ...
        // Search history by partial Rcode
        [HttpGet("search-history/{query}")]
        public async Task<IActionResult> SearchFileHistory(string query)
        {
            if (string.IsNullOrWhiteSpace(query))
            {
                return BadRequest(new { message = "Search query is required." });
            ...
            var history = await dbContext.RcodeFilesHistories
                .Where(h => h.Rcode.Contains(query))
                .OrderByDescending(h => h.TransferDate)
                .ToListAsync();

            if (!history.Any())
            {
                return NotFound(new { message = "No history found for matching files!" });
            }
        ...
        public async Task<IActionResult> GetAllFilesHistory()
        {
            var fifteenMonthsAgo = DateTime.UtcNow.AddMonths(-15);

            // 1. Get all history records older than 15 months
            var oldHistories = await dbContext.RcodeFilesHistories
                .Where(h => h.TransferDate < fifteenMonthsAgo)
                .ToListAsync();
        ...
        public async Task<IActionResult> AddFile([FromBody] AddRcodeFileDto addFileDto)
        {
            ...
            try
            {
                string eName = addFileDto.EName?.Trim();

                // Enforce valid values
                if (eName != "Colour Lab" && eName != "Library")
                {
                    eName = "Colour Lab"; // Default
                }

                string status = eName == "Library" ? "In" : "Out";

                // Validate EPF for Sample Team
                if (eName != "Library" && string.IsNullOrWhiteSpace(addFileDto.EpfNo))
                {
                    return BadRequest(new { message = "EPF No is required when Employee Name is not 'Library'." });
                }
                ...
                var fileEntity = new RcodeFile
                {
                    Rcode = addFileDto.Rcode,
                    EName = eName,
                    EpfNo = eName == "Library" ? "Library" : addFileDto.EpfNo,
                    ContactNo = string.IsNullOrWhiteSpace(addFileDto.ContactNo) ? "N/A" : addFileDto.ContactNo,
                    Status = status,
                    GetDate = addFileDto.GetDate,
                    UpdateDate = addFileDto.GetDate,
                    Department = addFileDto.Department
                ...
                return Ok(new { message = "File added successfully.", file = fileEntity });
            }
            catch (Exception ex)
            {
                return StatusCode(500, new { message = "Error adding file.", error = ex.Message });
